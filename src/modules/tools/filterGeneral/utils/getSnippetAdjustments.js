...
function getListOfRelevantAttrNames (snippets) {
    ...
    snippets.forEach(snippet => {
        ...
        else if (typeof snippet.attrName === "string") {
...
function getSnippetAdjustments (snippets, items, page, total) {
    // console.log("getSnippetAdjustments", page, total, Array.isArray(snippets), Array.isArray(items));
    if (
    ...
    snippets.forEach(snippet => {
        if (
        ...
        else if (snippet.type === "dropdown") {
            result[snippet.snippetId] = {
                value: valueByAttrName[snippet.attrName]
            };
        }
        else if (snippet.type === "slider" || snippet.type === "sliderRange") {
            const sortedValue = typeof snippet.attrName === "string" ? valueByAttrName[snippet.attrName].sort((a, b) => {
            ...
            result[snippet.snippetId] = {
                min: parseFloat(sortedValue[0]),
                max: parseFloat(sortedValue[sortedValue.length - 1])
            };
        }
