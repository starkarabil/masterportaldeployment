...
async function fetchRoutingOrsDirections ({
    ...
    try {
        ...
        if (e.response.status === 404) {
            throw new Error(i18next.t("modules.tools.routing.errors.noRouteFound"));
        }
        if (e.response && e.response.data && e.response.data.error) {
            if (e.response.data.error.code === 2003) {
                throw new Error(i18next.t("modules.tools.routing.errors.avoidAreaBig"));
            }
        }
        throw new Error(i18next.t("modules.tools.routing.errors.errorRouteFetch"));
    }
